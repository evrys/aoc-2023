...
PUZZLE_INPUT = """five3onelxjninenine45
...
def calibrated(lines: list[str]):
    numbers = ["zero", "one", "two", "three", "four",
               "five", "six", "seven", "eight", "nine"]

    total = 0
    for line in lines:
      matches = re.findall(rf'(?=([0-9]|{"|".join(numbers)}))', line)
      nums_in_line = [
        int(num) if num.isnumeric()
        else numbers.index(num)
        for num in matches
      ]
      total += int(f"{nums_in_line[0]}{nums_in_line[-1]}")
    print(total)


        
calibrated(PUZZLE_INPUT.split("\n"))